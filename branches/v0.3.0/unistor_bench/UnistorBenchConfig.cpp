...
int UnistorBenchConfig::loadConfig(string const & strConfFile){
	...
	m_strOpr =value;
	if ((m_strOpr != "add")&&(m_strOpr != "set")&&(m_strOpr != "update")&&(m_strOpr != "delete")&&(m_strOpr != "get")){
		snprintf(m_szError, 2047, "invalid data opr[%s] for [unistor_bench:data_opr], must be [add/set/update/delete/get", m_strOpr.c_str());
		return -1;
	}
