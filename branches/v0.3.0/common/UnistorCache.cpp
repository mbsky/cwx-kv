                                 ...
                                 CWX_UINT32 uiOldExpire)
{
    ...
    char key_buf[UnistorReadCacheItem::calBufCapacity(unKeyLen, 0, 0) + sizeof(UnistorWriteCacheItem)];
    UnistorWriteCacheItem* key = (UnistorWriteCacheItem*)key_buf;
    memcpy(key->m_szBuf, szKey, unKeyLen);
    key->m_unKeyLen = unKeyLen;

    CWX_UINT32 const uiCapacity = UnistorReadCacheItem::calBufCapacity(unKeyLen, uiDataLen, m_unAlign);
                              ...
                              CWX_UINT32 uiOldExpire)
{
    ...
    CWX_UINT32  const uiCapacity = UnistorReadCacheItem::calBufCapacity(unKeyLen, 0, m_unAlign);
                               ...
                               bool& bDel)
{
    ...
    char key_buf[UnistorReadCacheItem::calBufCapacity(unBeginKeyLen, 0, 0) + sizeof(UnistorWriteCacheItem)];
    UnistorWriteCacheItem* key = (UnistorWriteCacheItem*)key_buf;
...
///提交数据；0：成功；-1：失败
int UnistorWriteCache::commit(CWX_UINT64 ullSid){
    ...
    while(!pCache->m_bExit && pCache->m_bValid){
        if (pCache->m_writeCache->m_ullCommitSid){///有数据需要提交
            ...
            while(uiPos<pCache->m_writeCache->m_uiCommitEndPos){
                ...
                if (pItem->getCapacity() != UnistorReadCacheItem::calBufCapacity(pItem->getKeyLen(),
            ...
            if (pCache->m_bValid && index.size()){///如果有需要提交的数据
                ...
                if (0 != pCache->m_writeCache->m_fnEndWrite(pCache->m_writeCache->m_context, pCache->m_writeCache->m_ullCommitSid, pCache->m_szErr2K)){
                    pCache->m_bValid = false;
                    break;
                }
                ///将key从写cache中删除
                {
                    ...
                    CwxWriteLockGuard<CwxRwLock>  lock(&pCache->m_rwLock);
                    iter = index.begin();
                    while(iter != index.end()){
                       ...
                       CWX_UINT16 unAlign)
{
    ...
    if (m_uiWriteCacheByte){
        m_writeCache = new UnistorWriteCache(m_uiWriteCacheByte, m_fnEqual, m_fnLess, m_fnHash, NULL, unAlign);
        if (0 != m_writeCache->init(fnBeginWrite, fnWrite, fnEndWrite, context)) return -1;
    }
    if (m_ullReadCacheByte){
        m_readCache = new UnistorReadCache(m_ullReadCacheByte, m_uiReadCacheKeyNum, m_fnEqual, m_fnLess, m_fnHash, unAlign, NULL);
    }
    //启动commit线程
    if (m_writeCache){
        if(0 != ::pthread_create(&m_commitThreadId, NULL, UnistorCache::commitThreadMain, this)){
            m_bValid = false;
                            ...
                            bool  bCache)
{
    if (m_writeCache){
        ...
        if ((1 == ret) && m_readCache){
            if (bCache){
                m_readCache->insert(szKey, unKeyLen, szData, uiDataLen);
            }else{
                            ...
                            bool bNotExist)
{
    if (m_readCache){
        CwxWriteLockGuard<CwxRwLock>  lock(&m_rwLock);
        if (bNotExist){
            if (m_readCache->exist(szKey, unKeyLen)) return;
        }
        m_readCache->insert(szKey, unKeyLen, szData, uiDataLen);
    }
}
                         ...
                         bool& bDel)
{
    ...
    if ((0 == ret)  && m_readCache){
        UnistorReadCacheItem* data = m_readCache->fetch(szKey, unKeyLen, true);
        if (!data) return 0;
        if (uiDataLen < data->getDataLen()) return -1;
        memcpy(szData, data->getData(), data->getDataLen());
        uiDataLen = data->getDataLen();
        bDel = false;
        return 1;
    }
    return ret;
...
//-1: failure, 0: success
int UnistorCache::commit(CWX_UINT64 ullSid, char* szErr2K)
{///必须不能lock m_rwLock
    if (m_writeCache){
        if (!m_bValid){
        ...
        if (0 != m_writeCache->commit(ullSid)){
