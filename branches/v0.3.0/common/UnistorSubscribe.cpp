...
///比较函数
bool UnistorKeyAsciiLess::operator()(string const& key1, string const& key2) const{
    return UnistorStoreBase::getKeyAsciiLess()(key1.c_str(), key1.length(), key2.c_str(), key2.length())<0?true:false;
}

///是否订阅
bool UnistorSubscribeKey::isSubscribe(char const* key) const{
    map<string,string,UnistorKeyAsciiLess>::const_iterator iter = m_keys.begin();
    while(iter != m_keys.end()){
        if ((UnistorStoreBase::getKeyAsciiLess()(key, strlen(key), iter->first.c_str(), iter->first.length())>=0) &&
            ((UnistorStoreBase::getKeyAsciiLess()(key, strlen(key), iter->second.c_str(), iter->second.length())<0)|| !iter->second.length())) return true;
        iter++;
    }
    return false;
}
