...
public:
    char const*       m_begin; ///<开始位置，若为NULL，表示从store的开始位置（asc=true)或结束位置(asc=false)
    char const*       m_end; ///<结束位置，若为NULL不限定结束位置
    ...
    string           m_strBeginKey; ///<export的开始key
    string           m_strExtra; ///<export的extra
...
protected:
    ...
    static UNISTOR_KEY_GROUP_FN    m_fnKeyGroup;    ///<key的group分组获取函数
