...
public:
    enum{
        UNISTOR_WRITE_CACHE_MIN_SIZE = 1024 * 1024 * 64 ///<写cache的最小大小，为64M
    };
public:
    ///构造函数
...
private:
    ...
    CWX_UINT32                      m_uiSplitHalfPos; ///<内存分割点
    bool                            m_bFirstHalf; ///<是否当前使用上半区
    CWX_UINT32                      m_uiPos; ///<cache写到的当前位置
    CWX_UINT32                      m_uiCommitBeginPos; ///<commit的开始位置
    CWX_UINT32                      m_uiCommitEndPos;  ///<commit的结束位置
    CWX_UINT64                      m_ullCommitSid; ///<提交的sid
...
public:
    ...
    ///获取read cache的key的数量
    inline CWX_UINT32 getReadCacheKeyNum(){
        CwxReadLockGuard<CwxRwLock>  lock(&m_rwLock);
        return m_readCache?m_readCache->count():0;
    }
    
    ///获取read cache的空间
    inline  unsigned long int getReadCacheUsedSize(){
        CwxReadLockGuard<CwxRwLock>  lock(&m_rwLock);
        return m_readCache?m_readCache->size():0;
    }
...
private:
    ...
    UnistorReadCache*               m_readCache; ///<读cache            
    CwxRwLock                       m_rwLock; ///<读写锁，若为NULL表示没有锁
