...
int UnistorConfig::init(string const& strCnfFile){
	...
	//load common:store_flush_num
	if (m_cnf.getAttr("common", "store_flush_num", value)){
		m_common.m_uiStoreFlushNum = strtoul(value.c_str(), NULL, 10);
	}
	if (m_common.m_uiStoreFlushNum < UNISTOR_MIN_STORE_FLUSH_NUM){
		m_common.m_uiStoreFlushNum = UNISTOR_MIN_STORE_FLUSH_NUM;
	}
	if (m_common.m_uiStoreFlushNum > UNISTOR_MAX_STORE_FLUSH_NUM){
		m_common.m_uiStoreFlushNum = UNISTOR_MAX_STORE_FLUSH_NUM;
	}
