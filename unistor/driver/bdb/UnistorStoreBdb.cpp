...
int UnistorStoreBdb::importKey(UnistorTss* tss, ///<tss对象
...
int UnistorStoreBdb::syncImportKey(UnistorTss* tss, ///<线程的tss数据
                          ...
                          bool bKeyInfo)
{
    ...
    do{
        if (pBdbCursor->m_bStoreMore && !pBdbCursor->m_bStoreValue){///获取store的值
        ...
        if (!pBdbCursor->m_bStoreMore && !cursor.m_cacheCursor->m_bCacheMore) return 0; ///没有数据
            ...
            if (!pBdbCursor->m_bStoreMore){
                ...
                if (cursor.m_asc){
                    ...
                    if (ret < 0){
                        ...
                        bUserCache = true;
                        pBdbCursor->m_bStoreValue = false; ///store中的值作废，因为重复
                    }
                }else{
                    ret = keyStoreCmpLess(cursor.m_cacheCursor->m_szCacheKey,
                        cursor.m_cacheCursor->m_unCacheKeyLen,
    ...
    do{
        ...
        do{
            if (pCursor->m_bFirst){///第一次
                ...
                if (!cursor.m_unBeginKeyLen){///没有指定范围
                    if (cursor.m_asc){
                ...
                ret = keyStoreCmpLess(cursor.m_beginKey,
                    ...
                    bdb_key.size);
                if (!cursor.m_unBeginKeyLen) break; ///<一定取当前只
...
///检查订阅格式是否合法
bool UnistorStoreBdb::isValidSubscribe(UnistorSubscribe const& subscribe,///<订阅对象
...
int UnistorStoreBdb::_getSysKey(UnistorTss* , ///<线程tss对象
    ...
    do{
        ...
        if (0 == ret){
            ...
            if (cursor.m_cacheCursor->m_uiCacheDataLen){///不能做unSkipNum--
                if (keyStoreCmpLess(szKey, unKeyLen, cursor.m_cacheCursor->m_szCacheData, cursor.m_cacheCursor->m_uiCacheDataLen) >= 0){///已经结束
