...
int UnistorConnector::complete (set<int>& fds, CwxTimeouter* timeout){
    ...
    while(fds.size()){
        ...
        while(iter != fds.end()){
            ...
            if (*iter > select_width) select_width = *iter;
            iter++;
        }
        select_width++;
        result = CwxSocket::select(select_width, NULL, &handle_set, NULL, timeout, true);
        if (0 > result){
            return -1;
        }else if (0 == result){
            errno = ETIME;
        ...
        while(iter != fds.end()){
            if (FD_ISSET(*iter, &handle_set)){
                ...
                sockopt_ret = ::getsockopt (fd, SOL_SOCKET, SO_ERROR, &sock_err, &sock_err_len);
...
int UnistorConnector::connect (CwxINetAddr const& addr,
    ...
    do{
        ...
        for (i=0; i<unConnNum; i++){
            ...
            int result = ::connect (fd[i],
                reinterpret_cast<sockaddr *> (addr.getAddr ()),
                addr.getSize ());
