...
CWX_UINT32 g_uiVersion=0;
///-1：失败；0：help；1：成功
int parseArg(int argc, char**argv)
{
    CwxGetOpt cmd_option(argc, argv, "H:P:k:f:X:d:x:t:u:p:v:S:ch");
    int option;
    while( (option = cmd_option.next()) != -1)
    {
        ...
        {
        case 'h':
            printf("set a key or key's field.\n");
            ...
            printf("-c: not cache key in memory.\n");
            printf("-u: user name.\n");
            printf("-p: user password.\n");
            printf("-S: set sign. 0:set full key. 1:set field.\n");
            printf("-h: help\n");
            return 0;
        case 'H':
        ...
        case 'v':
            ...
            g_uiVersion = strtoul(cmd_option.opt_arg(), NULL, 10);
            break;
        case 'u':
            if (!cmd_option.opt_arg() || (*cmd_option.opt_arg() == '-'))
            {
...
int main(int argc ,char** argv)
{
	...
	do 
    {
		if (UNISTOR_ERR_SUCCESS != UnistorPoco::packRecvSet(
            &writer,
            block,
			0,
			key,
			...
			szErr2K))
		{
            printf("failure to pack set key package, err=%s\n", szErr2K);
            iRet = 1;
            break;
        }
        ...
        if (UnistorPoco::MSG_TYPE_RECV_SET_REPLY != head.getMsgType())
        {
        ...
        {
            printf("failure to set key, err_code=%d, err=%s\n", iRet, pErrMsg);
            iRet = 1;
            break;
        }
        iRet = 0;
        printf("success to set key[%s], field=%s, data=%s, version=%u field_num=%u\n",
            g_key.c_str(),
            g_field.length()?g_field.c_str():"",
			g_data.c_str(),
            uiVersion,
            uiFieldNum);
    } while(0);
