...
{
    CwxGetOpt cmd_option(argc, argv, "H:P:k:X:d:x:t:u:p:v:ch");
    int option;
...
int main(int argc ,char** argv)
{
	...
	do 
    {
		if (UNISTOR_ERR_SUCCESS != UnistorPoco::packRecvImport(
            &writer,
            block,
			0,
			key,
        ...
        iRet = 0;
        printf("success to import key[%s], data=%s, version=%u field_num=%u\n",
            g_key.c_str(),
			g_data.c_str(),
            uiVersion,
            uiFieldNum);
