...
private:
    ZkAdaptor*          m_zkHandler; ///<zk的handler
    string              m_strPath;  ///<锁的路径
    string              m_strPrex; ///<锁的前缀，node为[prex]-[session-id]-[seq]的格式
    struct ACL_vector*  m_acl;    ///<锁的权限
    void *              m_context;  ///<环境信息
    ZK_LOCK_COMPLETION  m_completion;  ///<锁变化的通知函数
    pthread_mutex_t     m_mutex;     ///<锁对象
    string              m_strSelfNode;   ///<自己的节点名字
    ...
    unsigned int          m_uiWatchType; ///<是否watch master。缺省wathe其前一个防止惊群
    watcher_fn          m_lock_func; ///<锁变化的回调function
    void*               m_lock_func_context; ///<锁变化的回调的context
